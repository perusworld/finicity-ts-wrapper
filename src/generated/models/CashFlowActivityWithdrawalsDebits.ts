/* tslint:disable */
/* eslint-disable */
/**
 * Open Banking
 * OpenAPI specification for Finicity APIs.  Open Banking solutions in the US are provided by Finicity, a Mastercard company.
 *
 * The version of the OpenAPI document: 1.13.9
 * Contact: apisupport@mastercard.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface CashFlowActivityWithdrawalsDebits
 */
export interface CashFlowActivityWithdrawalsDebits {
    /**
     * Date the withdrawal transaction was posted
     * @type {string}
     * @memberof CashFlowActivityWithdrawalsDebits
     */
    date: string;
    /**
     * Description of transaction
     * @type {string}
     * @memberof CashFlowActivityWithdrawalsDebits
     */
    transactionDescription?: string;
    /**
     * Amount of the withdrawal
     * @type {number}
     * @memberof CashFlowActivityWithdrawalsDebits
     */
    withdrawalsDebits: number;
}

/**
 * Check if a given object implements the CashFlowActivityWithdrawalsDebits interface.
 */
export function instanceOfCashFlowActivityWithdrawalsDebits(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "date" in value;
    isInstance = isInstance && "withdrawalsDebits" in value;

    return isInstance;
}

export function CashFlowActivityWithdrawalsDebitsFromJSON(json: any): CashFlowActivityWithdrawalsDebits {
    return CashFlowActivityWithdrawalsDebitsFromJSONTyped(json, false);
}

export function CashFlowActivityWithdrawalsDebitsFromJSONTyped(json: any, ignoreDiscriminator: boolean): CashFlowActivityWithdrawalsDebits {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'date': json['date'],
        'transactionDescription': !exists(json, 'transactionDescription') ? undefined : json['transactionDescription'],
        'withdrawalsDebits': json['withdrawalsDebits'],
    };
}

export function CashFlowActivityWithdrawalsDebitsToJSON(value?: CashFlowActivityWithdrawalsDebits | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'date': value.date,
        'transactionDescription': value.transactionDescription,
        'withdrawalsDebits': value.withdrawalsDebits,
    };
}

