/* tslint:disable */
/* eslint-disable */
/**
 * Open Banking
 * OpenAPI specification for Finicity APIs.  Open Banking solutions in the US are provided by Finicity, a Mastercard company.
 *
 * The version of the OpenAPI document: 1.13.9
 * Contact: apisupport@mastercard.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import type { ReportCustomField } from './ReportCustomField';
import {
    ReportCustomFieldFromJSON,
    ReportCustomFieldFromJSONTyped,
    ReportCustomFieldToJSON,
} from './ReportCustomField';
import type { StatementData } from './StatementData';
import {
    StatementDataFromJSON,
    StatementDataFromJSONTyped,
    StatementDataToJSON,
} from './StatementData';

/**
 * 
 * @export
 * @interface StatementReportConstraints
 */
export interface StatementReportConstraints {
    /**
     * 
     * @type {StatementData}
     * @memberof StatementReportConstraints
     */
    statementReportData: StatementData;
    /**
     * The `reportCustomFields` parameter is used when experiences are associated with a credit decisioning report.
     * 
     * Designate up to 5 custom fields that you'd like associated with the report when it's generated. Every custom field consists of three variables: `label`, `value`, and `shown`. The `shown` variable is "true" or "false".
     * * "true": (default) display the custom field in the PDF report
     * * "false": don't display the custom field in the PDF report
     * 
     * For an experience that generates multiple reports, the `reportCustomFields` parameter gets passed to all reports.
     * 
     * All custom fields display in the Reseller Billing API.
     * @type {Array<ReportCustomField>}
     * @memberof StatementReportConstraints
     */
    reportCustomFields?: Array<ReportCustomField>;
}

/**
 * Check if a given object implements the StatementReportConstraints interface.
 */
export function instanceOfStatementReportConstraints(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "statementReportData" in value;

    return isInstance;
}

export function StatementReportConstraintsFromJSON(json: any): StatementReportConstraints {
    return StatementReportConstraintsFromJSONTyped(json, false);
}

export function StatementReportConstraintsFromJSONTyped(json: any, ignoreDiscriminator: boolean): StatementReportConstraints {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'statementReportData': StatementDataFromJSON(json['statementReportData']),
        'reportCustomFields': !exists(json, 'reportCustomFields') ? undefined : ((json['reportCustomFields'] as Array<any>).map(ReportCustomFieldFromJSON)),
    };
}

export function StatementReportConstraintsToJSON(value?: StatementReportConstraints | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'statementReportData': StatementDataToJSON(value.statementReportData),
        'reportCustomFields': value.reportCustomFields === undefined ? undefined : ((value.reportCustomFields as Array<any>).map(ReportCustomFieldToJSON)),
    };
}

